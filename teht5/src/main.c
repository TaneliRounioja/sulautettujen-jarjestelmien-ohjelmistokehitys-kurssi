...
uint64_t timing_ns_red;
uint64_t timing_ns_green;
uint64_t timing_ns_yellow;
uint64_t timing_ns_uart;
uint64_t timing_ns_dispatch;
...
int main(void)
{
	...
	/*Saatiin 2 pisteeseen asti, tehtiin testit onko 6 inputtia, onko NULL ja 
	onko pelkästään nollia*/ 
...
static void uart_task(void *unused1, void *unused2, void *unused3)
{
	...
	while (true) {
        timing_start();
		timing_t uart_start_time = timing_counter_get();
			...
			// Character is newline, copy dispatcher data and put to FIFO buffer
				...
				if(uart_msg[0] == '0'){
					...
					if (ret > COMMAND_OK) {
						...
						printk("TIME arvo error");
					}
					else if (ret == TIME_LEN_ERROR)
					{
						
						printk("TIME pituus error");
					}
					else if (ret == TIME_ZERO_ERROR){
						printk("TIME arvo nolla");
					}
				}
                ...
                timing_t uart_end_time = timing_counter_get();
		timing_stop();
    	        timing_ns_uart = timing_cycles_to_ns(timing_cycles_get(&uart_start_time, &uart_end_time));
		k_msleep(10);
	}
	return 0;
}
...
static void dispatcher_task(void *unused1, void *unused2, void *unused3)
{
	while (true) {
        bool test = false;
        timing_start();
		timing_t dispatcher_start_time = timing_counter_get();
    ...
    if (test == true){
    printk("all tasks: %lld\n", timing_ns_yellow + timing_ns_green + timing_ns_red);
    printk("uart task time: %lld\n", timing_ns_uart);
    timing_t dispatcher_end_time = timing_counter_get();
    timing_stop();
    timing_ns_dispatch = timing_cycles_to_ns(timing_cycles_get(&dispatcher_start_time, &dispatcher_end_time));
    printk("dispatch task time: %lld\n", timing_ns_dispatch);
...
void red_led_task(void *, void *, void*) {
	...
	while (true) {
		...
		timing_stop();
    	        timing_ns_red = timing_cycles_to_ns(timing_cycles_get(&red_start_time, &red_end_time));
		//printk("Red task: %lld\n", timing_ns_red);
...
void blue_led_task(void *, void *, void*) {
	...
	while (true) {
			...
			timing_t green_end_time = timing_counter_get();
		        timing_stop();
    	                timing_ns_green = timing_cycles_to_ns(timing_cycles_get(&green_start_time, &green_end_time));
...
void yellow_led_task(void *, void *, void*) {
	...
	while (true) {
		...
		timing_stop();
    	        timing_ns_yellow = timing_cycles_to_ns(timing_cycles_get(&yellow_start_time, &yellow_end_time));
...
int time_parse(const char *time)
{
    ...
    if (!time) return TIME_LEN_ERROR;

    
    for (int i=0; i<6; i++) {
        if (time[i] < '0' || time[i] > '9') return TIME_VALUE_ERROR;
    }
    ...
    if (h==0 && m==0 && s==0) return TIME_ZERO_ERROR;
    if (h>23 || m>59 || s>59) return TIME_VALUE_ERROR;
